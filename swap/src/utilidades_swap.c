...
void cargar_configuracion_swap(char *archivo, t_config_swap *configuracion_swap) {
	...
	if (config_has_property(archivo_configuracion, "PUERTO_ESCUCHA")) {
		configuracion_swap->puerto_escucha = config_get_int_value(archivo_configuracion, "PUERTO_ESCUCHA");
	} else {
		configuracion_swap->puerto_escucha = DEFAULT_PUERTO_ESCUCHA;
	}

	if (config_has_property(archivo_configuracion, "NOMBRE_SWAP")) {
		strcpy(configuracion_swap->nombre_swap, config_get_string_value(archivo_configuracion, "NOMBRE_SWAP")); //Hago la copia para así despues usar config_destroy y que no explote todo
		//configuracion_swap->nombre_swap = config_get_string_value(archivo_configuracion, "NOMBRE_SWAP");
	} else {
		configuracion_swap->nombre_swap = DEFAULT_NOMBRE_SWAP;
	}

	if (config_has_property(archivo_configuracion, "CANTIDAD_PAGINAS")) {
		configuracion_swap->cantidad_paginas = config_get_int_value(archivo_configuracion, "CANTIDAD_PAGINAS");
	} else {
		configuracion_swap->cantidad_paginas = DEFAULT_CANTIDAD_PAGINAS;
	}

	if (config_has_property(archivo_configuracion, "TAMANO_PAGINA")) {
		configuracion_swap->tamano_pagina = config_get_int_value(archivo_configuracion, "TAMANO_PAGINA");
	} else {
		configuracion_swap->tamano_pagina = DEFAULT_TAMANO_PAGINA;
	}

	if (config_has_property(archivo_configuracion, "RETARDO_COMPACTACION")) {
		configuracion_swap->retardo_compactacion = config_get_int_value(archivo_configuracion, "RETARDO_COMPACTACION");
	} else {
	...
	if (config_has_property(archivo_configuracion, "RETARDO_ACCESO")) {
			configuracion_swap->retardo_acceso = config_get_int_value(archivo_configuracion, "RETARDO_ACCESO");
	} else {
